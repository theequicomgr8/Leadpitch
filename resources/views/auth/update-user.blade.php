							...
							<form class="form-horizontal form-label-left" role="form" method="POST" action="">
								...
								<div class="form-group manager" style="<?php echo ((isset($user->role) && ($user->role=="user" || $user->role=="manager"))?"":"display:none") ?>">
									...
									<select class="form-control col-md-7 col-xs-12" <?php echo (Auth::user()->current_user_can('super_admin')||Auth::user()->current_user_can('administrator') ||Auth::user()->current_user_can('manager'))?"name=\"manager\"":"disabled"; ?>>
									 ...
									 $managers = DB::table(Session::get('company_id').'_users')->where('role','!=','user')->where('role','!=','TL')->get();
										  if(!empty($managers)){
								...
								<!-- For TL -->
								<div class="form-group manager" style="<?php echo ((isset($user->role) && ($user->role=="user" || $user->role=="manager"))?"":"display:none") ?>">
									<label class="control-label col-md-3 col-sm-3 col-xs-12" for="manager">Team Leader</label>
									<div class="col-md-6 col-sm-6 col-xs-12">
											 
									<select class="form-control col-md-7 col-xs-12" <?php echo (Auth::user()->current_user_can('super_admin')||Auth::user()->current_user_can('administrator') ||Auth::user()->current_user_can('manager') || Auth::user()->current_user_can('TL'))?"name=\"TL\"":"disabled"; ?>>
											<option value="">Select Team Leader</option>	
										<?php
									 $tls = DB::table(Session::get('company_id').'_users')->where('role','TL')->get();
									 
										  if(!empty($tls)){
											  foreach($tls as $tldata){ ?>
											  
												  <option value='<?php echo $tldata->id ?>' <?php if(isset($edit_data) && $edit_data->TL== $tldata->id){ echo "selected"; }?>><?php echo $tldata->name.' ('.$tldata->role.')';  ?></option>
											<?php  }
										  }
										?>
