						...
						<div class="x_content">
							...
							<form id="update-course" data-parsley-validate class="form-horizontal form-label-left" method="POST" action="">
								<div class="form-group">
									...
									<div class="col-xs-6">
										{{ csrf_field() }}
										<select class="select2_bucketcounsellor form-control" name="counsellors" tabindex="-1">
									...
									{{ (isset($edit_data) && $edit_data->counsellors ==$user->id   ) ? "selected":"" }} @endif>{{ $user->name }}</option>
									 ...
									 <div class="col-xs-6">
										{{ csrf_field() }}
										<select class="form-control" name="tocounsellor" tabindex="-1">
										 <option value="">Please Counsellor</option>   
										@if(!empty($users))
												@foreach($users as $user)
