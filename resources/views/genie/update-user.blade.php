						...
						<div class="x_content">
							...
							<form class="form-horizontal form-label-left" role="form" method="POST" action="">
								...
								<div class="form-group {{ $errors->has('role') ? ' has-error' : '' }}">
									...
									<div class="col-md-6 col-sm-6 col-xs-12">
										<select id="role" class="form-control col-md-7 col-xs-12" <?php echo (Auth::user()->current_user_can('super_admin')||Auth::user()->current_user_can('administrator') )?"name=\"role\"":"disabled"; ?>>
											<?php
											$roles = [		
								...
								<div class="form-group">
									<label class="control-label col-md-3 col-sm-3 col-xs-12" for="capabilities">Capabilities </label>
									<div class="col-md-6 col-sm-6 col-xs-12">
										<?php
										...
										?>
										<select id="capabilities" class="form-control chosen-select col-md-7 col-xs-12" multiple <?php echo (Auth::user()->current_user_can('super_admin')||Auth::user()->current_user_can('administrator'))?"name=\"capabilities[]\"":"disabled"; ?>>
										<?php echo $html; ?>
										</select>
									</div>
								</div>
							 
								<div class="form-group manager" style="<?php echo ((isset($user->role) && $user->role=="user")?"":"display:none") ?>">
									<label class="control-label col-md-3 col-sm-3 col-xs-12" for="manager">Manager</label>
									<div class="col-md-6 col-sm-6 col-xs-12">
											 
									<select class="form-control col-md-7 col-xs-12" <?php echo (Auth::user()->current_user_can('super_admin')||Auth::user()->current_user_can('administrator'))?"name=\"manager\"":"disabled"; ?>>
										...
										<?php
										  ...
										  if(!empty($managers)){
											  foreach($managers as $manager){ ?>
												  ...
												  <option value='<?php echo $manager->id ?>' <?php if(isset($edit_data) && $edit_data->manager== $manager->id){ echo "selected"; }?>><?php echo $manager->name ?></option>
											<?php  }
										  }
										?>
