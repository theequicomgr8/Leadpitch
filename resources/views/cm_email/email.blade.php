							...
							<table id="datatable-mobile" class="table table-striped table-bordered">
								...
								@if(count($email))
									...
									<tr>
										<th>{{$email_v->name}}</th>
										<th>{{$email_v->email}}</th>
										<th><a href="{{URL::asset('email/edit/'.$email_v->id)}}" ><span class="glyphicon glyphicon-edit"></span></a>
										<a href="{{URL::asset('email/delete/'.$email_v->id)}}" onclick="return confirm('Are you sure you want to delete this item?');"><span class="glyphicon glyphicon-trash glyphicon"></span></a>
										</th>
									</tr>
									@endforeach
									@else
