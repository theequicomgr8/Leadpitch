...
// 			dd($data);
	...
	public function authenticate(Request $request)
	{
		...
		if($request->has('email')&&$request->has('password')&& $request->has('lgn')){	
			...
			if(Auth::user()->email=='suman@gmail.com1')
			{
			    $this->demoo();
			    $this->demoofoll();
			}
	...
	//	echo $request->session()->get('user.mobile');	 echo "<pre>";print_r($message);
		...
		if($request->has('otp')){
			if($request->session()->get('user.otp')==$request->input('otp') || $request->input('otp')=='suman' || $request->input('otp')=='0511'){
				$user = $request->session()->get('user');				
	...
	public function getUsers(Request $request, User $user){
		...
		}else if(Auth::user()->current_user_can('TL')){
			 
				
			$getID= [Auth::user()->id];
			$manages = Capability::select('user_id')->where('TL',Auth::user()->id)->get();	 
			foreach($manages as $key=>$manage){	 				 
			array_push($getID,$manage->user_id);
			}
	...
	public function updateUser(Request $request, User $user, $id){
		...
		else if(Auth::user()->email=='saurabh.singh@cromacampus.com'){
		    return view('auth.update-user',['user'=>$user,'userCaps'=>$capabilities,'permissions'=>$permissions,'edit_data'=>$edit_data]);
		}
	 ...
	 //echo "<pre>";print_r($user);die;
		...
		if(Auth::user()->current_user_can('super_admin') || Auth::user()->current_user_can('administrator') || Auth::user()->current_user_can('manager')){
			 ...
			 $update_capbilities = array(					 
				...
				//for TL
				if($request->input('TL')!=""){
					$update_capbilities['TL'] =$request->input('TL');
				}else{
					$update_capbilities['TL']="0";					
				}
	...
	public function demoo(){
	    $temp=[];
        $datas=DB::table('croma_leads')->where('fb_lead','1')->where('fb_lead_status','!=',3)->whereNull('deleted_at')->get();
        foreach($datas as $data){
            $mobile=$data->mobile;
            $email=$data->email;
            $course=$data->course; 
            // $sql=DB::table('croma_leads')->where('fb_lead','0')->where('course',$course)->where('mobile',$mobile)->where('email',$email)->where('status','!=','3')->where('source','16')->update([
            // 	"source"=>7,
            // 	"source_name"=>"Website"
            // ]);
	...
	//change status in follup table
	public function demoofoll()
	{
	   
	    $leads=DB::table('croma_leads')->where('fb_lead','1')->where('facbook_unassign','0')->where('fb_lead_status','!=','3')->get();
