   ...
   /**
	...
	{      
		...
		if($request->has('email')&&$request->has('password')&& $request->has('lgn')){
			...
			if($user){
				if (Hash::check(trim($request->input('password')), $user->password)) {
					...
					$request->session()->put('client.password', $request->input('password'));
					$request->session()->put('client.remember', $request->input('remember'));
					$request->session()->put('client.name', $user->name);		
					$request->session()->put('client.id', $user->id);		
					$request->session()->put('client.role', $user->role);		
				...
				/*  if (Auth::attempt(['email' => $user['email'], 'password' => $user['password']], $user['remember'])) {
					...
					return view('genie.mobile',compact('email','mobile'));
		...
		if($request->has('mobile') && $request->input('mobile') != ''){
			$request->session()->put('client.mobile', $request->input('mobile'));
	...
	//	echo $request->session()->get('user.mobile');	 echo "<pre>";print_r($message);
			...
			$send = sendSMS($request->session()->get('client.mobile'),$message);
			...
			$request->session()->put('client.otp_to_email', $request->input('otp_to_email'));
			$otp = mt_rand(100000, 999999);
			$request->session()->put('client.otp', $otp);
			$message = "{$otp} is Lead Portal Verification Code for {$request->session()->get('user.name')}.";
...
//echo "iiner";echo $request->session()->get('user.otp').'==';echo $request->input('otp');die;
	...
	public function getOTP(Request $request)
	{
		...
		if($request->session()->has('client.email') && $request->session()->has('client.password') && ($request->session()->has('client.mobile') || $request->session()->has('client.otp_to_email')))
			return response()->view('genie.otp');
		else
	...
	public function getRegister(){
		...
		return response()->view('genie.register');
	...
	public function clientDashboard()
	{
		...
		return view('genie/client_dashboard');
		}else{
			return redirect('/genie');
		}
	}
	...
	{   
		...
		return view('genie.list-client',compact('clients'));
		}else{
		return redirect('/genie');
		}
	}
